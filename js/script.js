...
resetBtn.addEventListener('click',(event)=>{
    ...
    passContainer.classList.add('animate__animated','animate__pulse','animate__faster');
    event.preventDefault();
    addToHistory(passContainer.textContent);
    setPassword();
    setTimeout(() => {
        passContainer.classList.remove('animate__animated','animate__pulse','animate__faster');
...
function getDifferentChar(pass,fnc){
    ...
    //console.log(result,pass[pass.length-1]||0,letter);
    return letter;
}
...
function addToHistory(pass){
    ...
    element.classList.add('m-0','d-flex', 'align-items-center', 'justify-content-center');
    element.style.height = '50px';
    let copyE =`${escapeHtml(pass)}<a href="javascript:void(0)" id="copyBtn" data-clipboard-text="${pass}" class="link-dark bi bi-clipboard mx-1 stretched-link"></a>`;
